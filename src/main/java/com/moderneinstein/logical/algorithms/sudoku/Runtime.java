...
public static boolean feasible(Integer[][] board){
        ...
        boolean ret = Validator.validate(sheet);
        return ret;
    }
...
public static void judge(Integer[][] filt,int a,boolean state){
        ...
        Integer x = 0;  
        Integer y = 0; 
        Integer[][] var3 = Filter.filter3(filt,a);
        //System.out.println(Arrays.deepToString(var3));
        for(int i = 0;i<var1;i++){
            for(int j = 0;j<var2;j++){
                if(filt[i][j]==0&&var3[i][j]!=0&&Extensions.singleCell(var3,i,j)){
                    count1++;
                    x=i;
                    y=j;
                    filt[i][j]=a;
          ...
          //  for(int j =0;j<var2;j++){
               ...
               // if(judge(var3,k)==true){
                ...
                //else{
                    ...
                    Runtime.impose(sheet);
                    if(Runtime.feasible(sheet)){
                        for(int i=0;i<var1;i++){
                            for(int j=0;j<var2;j++){
                                board[i][j]=sheet[i][j];
                            }
